...
###### Исправлено для печатания другим шрифтом
...
#        pil_fsize = 16
#        fnt = ImageFont.truetype("DejaVuSans.ttf", pil_fsize)
#        d = ImageDraw.Draw(pil_img)
#        d.text((c1[0]+5, c1[1] - 17), label, font=fnt, fill=(225,255,255,0))  #
#        cv2_img = cv2.cvtColor(np.asarray(pil_img), cv2.COLOR_RGB2BGR)
#        img[:] = cv2_img[:]     
    
###### Исправлено для расположения текста внутри рамки          
        tf = max(tl - 1, 1)  # font thickness
        #t_size = (114,16) #cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        t_size = cv2.getTextSize(label, 1, fontScale=tl/2.87, thickness=tf)[0]  # было t1/3
        c2 = c1[0] + t_size[0]-t_size[0]//4, c1[1] + t_size[1] + 12 #c1[0] + t_size[0]-10, c1[1] - t_size[1]-10
        c1 = c1[0], c1[1] - 1 #c1 = c1[0], c1[1]-1
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        pil_img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        pil_fsize = 18
        fnt = ImageFont.truetype("models/DejaVuSans.ttf", pil_fsize)
        d = ImageDraw.Draw(pil_img)
        d.text((c1[0]+5, c1[1] - 0), label, font=fnt, fill=(225,255,255,0))  #было -17  - высота расположения текста
        cv2_img = cv2.cvtColor(np.asarray(pil_img), cv2.COLOR_RGB2BGR)
        img[:] = cv2_img[:]
